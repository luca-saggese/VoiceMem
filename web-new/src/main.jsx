...
function VoiceApp({ user, onLogout }) {
  ...
  return <main className={`app ${collapsed ? 'rail-collapsed' : ''}`}>
    <Sidebar collapsed={collapsed} onCollapse={() => setCollapsed((value) => !value)} sessions={vm.sessions} activeId={vm.sessionId} onSelect={vm.setSessionId} onNew={vm.createSession} user={user} onLogout={onLogout} />
