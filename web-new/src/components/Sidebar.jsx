...
export function Sidebar({
  ...
  const addDevice = async (event) => {
    ...
    if (!/^(\d{6}|[0-9a-fA-F]{12})$/.test(deviceId)) {
      setDeviceError(
        "Inserisci il codice di 6 numeri oppure il MAC di 12 caratteri esadecimali.",
      );
      return;
    }
    if (!deviceName.trim()) {
      setDeviceError("Inserisci un nome per il device.");
      return;
    }
    setDeviceBusy(true);
    setDeviceError("");
    try {
      const response = await fetch("/api/devices", {
        ...
        body: JSON.stringify({ device_id: deviceId, name: deviceName.trim() }),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok)
        throw new Error(data.detail || "Impossibile registrare il device");
      ...
      {!collapsed && (
        <>
          <div className="side-search">
            <input placeholder="Cerca conversazioni" />
          </div>
          <div className="side-scroll">
            <div className="side-label device-label">
              Devices
              <button
                className="section-add"
                onClick={openDeviceModal}
                title="Aggiungi device Xiaozhi"
              >
                <Plus size={15} />
              </button>
            </div>
            <ul className="device-list">
              ...
              {!devices.length && (
                <li className="device-empty">Nessun device registrato</li>
              )}
            </ul>
            ...
            <ul className="sess">
              {sessions.map((session) => (
                ...
                >
                  ...
                  <span className="n">{session.turns.length}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          {deviceModalOpen && (
            ...
            >
              <form className="device-modal" onSubmit={addDevice}>
                <div className="device-modal-head">
                  <div>
                    <span className="auth-mark">XIAOZHI</span>
                ...
                <p className="device-modal-sub">
                  Registra il dispositivo associato al tuo account.
                </p>
                {deviceError && (
                  <div className="device-form-error">{deviceError}</div>
                )}
                <label>
                  Codice activation o MAC
                  <input
                    value={deviceId}
                    onChange={(event) =>
                      setDeviceId(
                    ...
                    inputMode="text"
                    pattern="([0-9]{6}|[0-9a-fA-F]{12})"
                    maxLength="12"
                    placeholder="14C19FD1CC98"
                    autoFocus
                  />
                </label>
                <label>
                ...
                <div className="device-modal-actions">
                  ...
                  <button
                    ...
                    disabled={deviceBusy}
                  >
                    {deviceBusy ? "Salvataggio..." : "Aggiungi device"}
                  </button>
                </div>
              </form>
            </div>
...
export function SpaceControls({ spaces, spaceId, onSpaceChange, onNewSpace }) {
  return (
    <div className="export">
      ...
      <button className="btn sm">Scarica</button>
    </div>
  );
}
